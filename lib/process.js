...
async function worker ({ file_location, detail, ordering, feature, project_id, process_id }) {
  ...
  try {
    ...
    // process
    await processor({ imgDir: file_location, outDir: file_location + 'models/', filename: 'model', detail, order: ordering, feature })

    // convert
    await convert({ file_location })

    // salvare il i modelli e i relativi file
    // leggo la cartella /models e salvo tutti i file
    const { files } = await promises.readdir(file_location + 'models/')
    for (let i = 0; i < files.length; i++) {
      const file = files[i]
      const { data: _dataFiles, error: _errorFiles } = await supabase.storage
        .from('models')
        .upload(`${file_location}models/${file}`, `${file_location}models/${file}`)
      console.log('upload', _dataFiles, _errorFiles)
    }
